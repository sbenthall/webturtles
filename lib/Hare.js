 ...
 *  Hare
 *
 *  Hare's are like Turtles, but with some important differences.
 *
 *  TODO: Hare's bodies are thinner and rotate with orientation.
 *  TODO: Hares have smaller heads with have big ears
 *  TODO: Hares do not move backwards.  Instead, they have a
 *        command, 'ffwd', which moves them forward two cells
 *        at once.
 *  TODO: When Hares move, they "hop" (their sprite moves in an
 *         arc) rather than walk.
 *
 *  Working on Hare should help figure out how to generalize the
 *  Turtle code to allow for more Creatures.
 */


function Hare(col, row, color, world, size, headSize, myId) {
    this.world = world;
    this.busy = 0;
...
tablehare = function(world, col, row, color, id) {
    var t = new Hare(col, row, color,
		       world, 10, 5, id);
