...
function World(tableSelector, canLandOnFunc) {
    ...
    this.addCreature = function(creatureType,row,col,color,id){
	id = id || "turtle-" + this.turtles.length;
        var c = new creatureType(col, row, color,
	    this, 10, 5, id);

        c.setOrientation = function(x, y) {
	    c.or.x = x; c.or.y = y;
	    c.head.attr(fixHead(c.body.attr("cx"),
			        c.body.attr("cy"),
			        c));
        };

	this.turtles.push(c);
	c.initQueue("turtleCmd-" + id);
    };
